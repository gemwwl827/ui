...
# ✅ PATCH：将指定申请单号的所有记录设为目标状态（默认草稿 = 1），并返回旧值（用于还原）
def patch_core_asset_record_type(request_no: str, target_type: int = 1) -> int:
    """
    更新 CORE_ASSET_RECORD 表中某个申请单号的所有记录，将 RECORD_TYPE 更新为指定值（默认设为草稿 1）
    ✅ 同时返回原始 RECORD_TYPE（用于恢复）

    :param request_no: 申请单编号，如 'REQ-20240530-00028'
    :param target_type: 目标状态值，默认 1（草稿）
    :return: int，原始 RECORD_TYPE（假设所有记录一致，仅取第一个）
    """
    conn = get_dm_conn()
    cursor = conn.cursor()

    # ✅ 查询原始 RECORD_TYPE 值（用于后续恢复）
    cursor.execute("SELECT RECORD_TYPE FROM CORE_ASSET_RECORD WHERE REQUEST_NO = ?", (request_no,))
    result = cursor.fetchone()
    original_type = result[0] if result else None

    # ✅ 执行 PATCH 更新语句（仅更新与目标不同的记录）
    cursor.execute("""
        UPDATE CORE_ASSET_RECORD
        SET RECORD_TYPE = ?
        WHERE REQUEST_NO = ? AND RECORD_TYPE <> ?
    """, (target_type, request_no, target_type))

    conn.commit()
    cursor.close()
    conn.close()

    print(f"✅ PATCH 完成：申请单 {request_no} 的 RECORD_TYPE 已设为 {target_type}（原值：{original_type}）")
    return original_type


# ✅ RESTORE：将指定申请单号的 RECORD_TYPE 还原为原始值
def restore_core_asset_record_type(request_no: str, original_type: int):
    """
    将指定申请单号下所有记录的 RECORD_TYPE 恢复为传入的原始值

    :param request_no: 申请单编号
    :param original_type: 之前的 RECORD_TYPE（由 patch 时获取）
    """
    conn = get_dm_conn()
    cursor = conn.cursor()
    ...
    cursor.execute("""
        ...
        SET RECORD_TYPE = ?
        WHERE REQUEST_NO = ?
    """, (original_type, request_no))

    conn.commit()
    cursor.close()
    conn.close()

    print(f"🔁 数据库恢复成功：申请单 {request_no} 的 RECORD_TYPE 已恢复为 {original_type}")
...
def delete_approval_dm_data(request_no: str):
    ...
    try:
        ...
        # 🔧修改：如果未查到 EXEC_ID，尝试通过 BIZ_INST_NO 再查一次
        if not exec_ids and biz_inst_no:
            cursor.execute("SELECT EXEC_ID FROM CORE_ASSET_RECORD WHERE BIZ_INST_NO = ? AND EXEC_ID IS NOT NULL",
                           (biz_inst_no,))
            exec_id_rows = cursor.fetchall()
            exec_ids = [row[0] for row in exec_id_rows]
            print(f"🔁 通过 BIZ_INST_NO 补充查得 EXEC_ID 列表: {exec_ids}")
